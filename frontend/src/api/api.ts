...
export function createBhivApi(client: ApiClient) {
  return {
    ...
    summarize: (req: SummarizeRequest) =>
      client.request<SummarizeResponse>({
        method: 'POST',
        path: '/api/summarize',
        json: req,
      }),
    ...
    classifyTask: (req: TaskClassificationRequest) =>
      client.request<TaskClassificationResponse>({
        method: 'POST',
        path: '/api/task',
        json: req,
      }),
    ...
    similarity: (req: SimilarityRequest) =>
      client.request<SimilarityResponse>({
        method: 'POST',
        path: '/api/embed/similarity',
        json: req,
      }),

    // ===== Decision Hub / Orchestration =====
    decisionHub: (args: {
      input_text: string
      platform?: string
      ...
      return client.request<DecisionHubResponse>({
        method: 'POST',
        path: '/api/decision_hub',
        formData: fd,
        timeoutMs: 60_000,
      })
    },
    ...
    // ===== RL =====
    rlAction: (req: RLActionRequest) =>
      client.request<RLActionResponse>({
        method: 'POST',
        path: '/api/rl_action',
        json: req,
      }),
    ...
    externalLlm: (req: ExternalLLMRequest) =>
      client.request<ExternalLLMResponse>({
        method: 'POST',
        path: '/api/external_llm',
        json: req,
      }),

    externalApp: (req: ExternalAppRequest) =>
      client.request<ExternalAppResponse>({
        method: 'POST',
        path: '/api/external_app',
        json: req,
      }),

    // ===== Voice =====
    voiceTts: (req: VoiceTTSRequest) =>
      client.request<VoiceTTSResponse>({
        method: 'POST',
        path: '/api/voice_tts',
        json: req,
        timeoutMs: 60_000,
    ...
    voiceStt: (file: File) => {
      const fd = new FormData()
      fd.append('file', file)
      return client.request<VoiceSTTResponse>({
        method: 'POST',
        path: '/api/voice_stt',
        formData: fd,
        timeoutMs: 60_000,
    ...
    // ===== BHIV =====
    bhivRun: (req: BHIVRequest) =>
      client.request<BHIVResponse>({
        method: 'POST',
        path: '/api/bhiv/run',
        json: req,
      }),
